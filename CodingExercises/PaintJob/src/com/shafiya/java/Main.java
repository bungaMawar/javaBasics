...
//        1. Write a method named getBucketCount with 4 parameters. The first parameter should be named width of
...
//        The method needs to return a value of type int that represents the number of buckets that Bob
//        needs to buy before going to work. To calculate the bucket count, refer to the notes below.
//        If one of the parameters area or areaPerBucket is less or equal to 0, the method needs to return
//        -1to indicate an invalid value.
//         If all parameters are valid, the method needs to calculate the number of buckets and return it.
    ...
    public static int getBucketCount(double width, double height, double areaPerBucket, int extraBuckets){
        ...
        if(areaOfWall <= areaPerBucket){
            bucketsNeeded = 1;
        }
        else{
            bucketsNeeded = areaOfWall/areaPerBucket;//13.2
            if((bucketsNeeded * 10) % 10 != 0){//132 % 10 = 2
                bucketsNeeded = (bucketsNeeded * 10) + 10;//142
                int tempNumber = (int) bucketsNeeded % 10;//2.0
                bucketsNeeded = bucketsNeeded - (tempNumber);//140
                bucketsNeeded = bucketsNeeded/10;//14
            }
        }
        bucketsToBuy = (int) bucketsNeeded - extraBuckets;
        if(bucketsToBuy < 0){
            bucketsToBuy = 0;
        }
        return bucketsToBuy;
    ...
    public static int getBucketCount(double area, double areaPerBucket){
        ...
        double bucketsNeeded = 0;
        bucketsNeeded = area/areaPerBucket;//13.2
        if((bucketsNeeded * 10) % 10 != 0){//132 % 10 = 2
            bucketsNeeded = (bucketsNeeded * 10) + 10;//142
            int tempNumber = (int) bucketsNeeded % 10;//2.0
            bucketsNeeded = bucketsNeeded - (tempNumber);//140
